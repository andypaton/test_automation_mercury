...
public class InitialFundingRequestsPage extends Base_Page<InitialFundingRequestsPage>{
    ...
    private static final String GRID_ROW_XPATH = GRID_XPATH + "//td[contains(text(),'%s')]";
    ...
    // Page methods
    public InitialFundingRequestsPage(WebDriver driver) {
        super(driver);
    ...
    // Page Interactions
    public InitialFundingRequestsPage searchJobs(String searchQuery) {
    ...
    public InitialFundingRequestsApprovalPage openInitalFundingRequest(String jobReference) throws InterruptedException {
        By by = By.xpath(String.format(GRID_ROW_XPATH, jobReference));
        POHelper.isLoaded().isAngularFinishedProcessing().isFluentElementIsVisible(by);
        POHelper.clickJavascript(by);
        return PageFactory.initElements(driver, InitialFundingRequestsApprovalPage.class).get();
    }
    ...
    public InitialFundingRequestsPage tableSort(String header, boolean isAscending) {
        scrollTo(0);
