...
public class MultiQuoteAwaitingQuotePage extends Base_Page<MultiQuoteAwaitingQuotePage>{

    private static final Logger logger = LogManager.getLogger();

    // Page title
    private static final String PAGE_TITLE = "Multi-Quotes Awaiting Quote";
    ...
    private static final String TABLE_ROW_VIEW_QUOTE_XPATH = TABLE_BODY_XPATH + "//tr[contains(@class, 'qas-can-click')]";
    ...
    // Page methods
    public MultiQuoteAwaitingQuotePage(WebDriver driver) {
        super(driver);
    ...
    // Page Interactions
    public MultiQuoteAwaitingQuotePage searchJobs(String searchQuery) {
    ...
    public QuoteAwaitingResourceQuotePage viewQuoteJobRequest(Integer jobReference) {
        By by = By.xpath(String.format(TABLE_ROW_VIEW_QUOTE_XPATH, jobReference));
        POHelper.isLoaded().isAngularFinishedProcessing().isFluentElementIsVisible(by);
        driver.findElement(by).click();
        return PageFactory.initElements(driver, QuoteAwaitingResourceQuotePage.class).get();
    }

    public MultiQuoteAwaitingQuotePage tableSort(String header, boolean isAscending) {
        scrollTo(0);
