...
public class CustomCucumber extends ParentRunner<FeatureRunner> {
    ...
    private void addRunnerTag(RuntimeOptions runtimeOptions) {
        final String tagsToAdd = System.getProperty("add.tags", "");
        if ( !tagsToAdd.isEmpty() ) {
            String[] tags = tagsToAdd.split(";");
            for (String tag : Arrays.asList(tags)) {
                runtimeOptions.getFilters().add(tag.trim());
            }

            for (Object obj : runtimeOptions.getFilters()) System.out.println("*** cucumber tag: " + obj.toString());
        }
    }
