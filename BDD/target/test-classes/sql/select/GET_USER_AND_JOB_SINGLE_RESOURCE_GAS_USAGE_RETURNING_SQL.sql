...
FROM Job j INNER JOIN ResourceAssignment ra ON j.Id = ra.JobId   
    ...
    LEFT JOIN GasRequiredToReturn grtr ON grtr.id = sv.Id
    LEFT JOIN LeakSiteRequiredToReturn lsrtr ON lsrtr.id = sv.Id
WHERE 
   ...
   AND ( ( 
            CAST(:isRequiredToReturn AS bit)  = 1 
            and (grtr.IsRequiredToReturn = 1 or   
                 lsrtr.IsRequiredToReturn = 1)
           )
          OR
          ( 
           CAST(:isRequiredToReturn AS bit)  = 0 
            and (grtr.IsRequiredToReturn IS NULL or   
                 lsrtr.IsRequiredToReturn IS NULL)
          )
        )
